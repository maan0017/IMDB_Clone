...
func RegisterUser(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to check the existing user"})
			return
		}
		if count > 0 {
...
func LoginUser(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// c.JSON(http.StatusOK, gin.H{"message": "user logged in successfully", "access_token": token, "refresh_token": refreshToken})
